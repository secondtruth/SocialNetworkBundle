...
class FriendshipControllerTest extends WebTestCase
{
    ...
    public function testNoFriendAction()
    {
        ...
        $this->assertCount(1, $crawler->filter('body:contains("kiboko_social.socialnetwork.no_friend")'));
    }
    ...
    public function testHasFriendsAction()
    {
        ...
        $crawler = $client->request('GET', '/friends/');
        $this->assertCount(0, $crawler->filter('body:contains("kiboko_social.socialnetwork.no_friend")'));
    }
    ...
    public function testAddDisabledFriendAction()
    {
        ...
        $crawler = $client->submit($form, ['search' => 'userDisabled']);
        $this->assertCount(1, $crawler->filter('body:contains("kiboko_social.socialnetwork.add.no_user_found")'));
    }
    ...
    public function testAddExistingFriendAction()
    {
        ...
        $crawler = $client->request('GET', '/friends/searchToAdd');
        $form = $crawler->filter('form[action$="friends/searchToAdd"] button[name="searchSubmit"]')->form();
        $crawler = $client->submit($form, ['search' => 'user3']);
        $this->assertCount(1, $crawler->filter('body:contains("kiboko_social.socialnetwork.add.no_user_found")'));
    }
    ...
    public function testAddWildcardFriendAction()
    {
        ...
        $crawler = $client->request('GET', '/friends/searchToAdd');
        $form = $crawler->filter('form[action$="friends/searchToAdd"] button[name="searchSubmit"]')->form();
        $crawler = $client->submit($form, ['search' => 'user']);
    ...
    public function testAddFriendAction()
    {
        ...
        $messengerExtension = $client2->getContainer()->get('kiboko_social_network.twig.messenger_extension');
        $this->assertSame(0, $messengerExtension->nbOfUnreadMessage($user2));
        ...
        $crawler = $client1->submit($formSearch, ['search' => 'user2']);
        ...
        $this->assertSame('kiboko_social.socialnetwork.invitation.email.subject', $message->getSubject());

        // Messenger message
        $this->assertSame(0, $messengerExtension->nbOfUnreadMessage($user1));
        $this->assertSame(1, $messengerExtension->nbOfUnreadMessage($user2));

        $this->assertTrue($client1->getResponse()->isRedirect('/friends/'));
        $crawler = $client1->followRedirect();

        $this->assertCount(1, $crawler->filter('body:contains("kiboko_social.socialnetwork.invitation.success_msg")'));
    }
...
//        $client2->enableProfiler();
        ...
        $this->assertCount(0, $crawler2->filter('body:contains("kiboko_social.socialnetwork.invitation.asking")'));
...
//        $client1->enableProfiler();
        ...
        $this->assertCount(0, $crawler1->filter('ol.myFriends li span:contains("kiboko_social.socialnetwork.pending")'));
        ...
        $crawler1 = $client1->submit($formSearch, ['search' => 'user2']);
        ...
        $this->assertCount(1, $crawler1->filter('ol.myFriends li span:contains("kiboko_social.socialnetwork.pending")'));
        ...
        $crawler2 = $client2->request('GET', '/friends/');
        $this->assertSame(0, $messengerExtension->nbOfUnreadMessage($user1));
        $this->assertCount(1, $crawler2->filter('body:contains("kiboko_social.socialnetwork.invitation.asking")'));
        ...
        $this->assertSame(1, $mailCollector2->getMessageCount());
        $collectedMessages = $mailCollector2->getMessages();
        $message = $collectedMessages[0];
        $this->assertSame('kiboko_social.socialnetwork.accept.email.subject', $message->getSubject());
    ...
    public function testRefuseFriendshipAction()
    {
        ...
        $buttonYes = $crawler2->selectButton('kiboko_social.socialnetwork.yes');
        $form = $buttonYes->form();
        $client2->submit($form);
        ...
        $collectedMessages = $mailCollector2->getMessages();
        $message = $collectedMessages[0];
        $this->assertSame('kiboko_social.socialnetwork.refuse.email.subject', $message->getSubject());
    ...
    public function testRemoveFriendshipAction()
    {
        ...
        $client1 = $this->getUserLoggedClient('user1', 'user1');
        $user1 = $client1->getContainer()->get('security.context')->getToken()->getUser();
        $messengerExtension = $client3->getContainer()->get('kiboko_social_network.twig.messenger_extension');
        $crawler1 = $client1->request('GET', '/friends/');

        // Messenger message
        $this->assertSame(0, $messengerExtension->nbOfUnreadMessage($user1));
        $this->assertSame(0, $messengerExtension->nbOfUnreadMessage($user3));
        ...
        // We confirm refusal action
        $buttonYes = $crawler1->selectButton('kiboko_social.socialnetwork.yes');
        $form = $buttonYes->form();
        $client1->submit($form);

        // we check accept email
        $mailCollector1 = $client1->getProfile()->getCollector('swiftmailer');
        $this->assertSame(1, $mailCollector1->getMessageCount());
        $collectedMessages = $mailCollector1->getMessages();
        $message = $collectedMessages[0];
        $this->assertSame('kiboko_social.socialnetwork.remove.email.subject', $message->getSubject());

        // Messenger message
        $this->assertSame(0, $messengerExtension->nbOfUnreadMessage($user1));
        $this->assertSame(1, $messengerExtension->nbOfUnreadMessage($user3));
