...
class ResettingControllerTest extends WebTestCase
{
    ...
    public function testRequestAction()
    {
        ...
        $this->assertSame('kiboko_social.socialnetwork.lost_password.legend', $crawler->filter('form[action$="resetting/send-email"] legend')->text());
    }
    ...
    public function testSendEmailWithEmpyFormAction()
    {
        ...
        $this->assertCount(1, $crawler->filter('div.alert.alert-error:contains("kiboko_social.socialnetwork.lost_password.email_not_found")'));
    }
    ...
    public function testSendEmailWithWrongEmailAction()
    {
        ...
        $data = ['username' => 'userNotExits@example.com'];
        $crawler = $client->submit($form, $data);
        $this->assertCount(1, $crawler->filter('div.alert.alert-error:contains("kiboko_social.socialnetwork.lost_password.email_not_found")'));
    }
    ...
    public function testSendEmailAction()
    {
        ...
        $this->assertSame('user1@example.com', key($message->getTo()));
    ...
    public function testSendEmailAlreadySendAction()
    {
        ...
        $crawler = $client->request('GET', '/resetting/request');
        $crawler = $client->submit($form, $data);
        $this->assertSame('resetting.password_already_requested', $crawler->filter('div.alert.alert-error')->text());
    }
    ...
    public function testCheckEmailWithEmptyFormAction()
    {
        ...
        $user = $container->get('doctrine')->getEntityManager()->getRepository('KibokoSocialNetworkBundle:User')->findOneBy(['email' => $data['username']]);
        $url = $container->get('router')->generate('fos_user_resetting_reset', ['token' => $user->getConfirmationToken()]);
    ...
    public function testCheckEmailWithBadValuesFormAction()
    {
        ...
        $form = $crawler->filter('form[action$="'.$url.'"] button[type="submit"]')->form();
        $crawler = $client->submit($form, $newData);
        $this->assertCount(1, $crawler->filter('div.alert.alert-error:contains("kiboko_social.socialnetwork.lost_password.password_no_match")'));
        $security = $client->getContainer()->get('security.context');
        $this->assertFalse($security->isGranted('ROLE_USER'));
    }
